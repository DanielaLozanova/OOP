...
{
	material.print();
	cout << "Weight of element: " << getWeight();
}
...
double Furniture::costShiping() const
{
	Element* elements = getElements();
	unsigned int cap = getCapacity();
	double sum = 0.0;
...
double Shoes::costShiping() const
{
	if (tryS == true)
	{
...
void CourierCompany::addItemToCC(const Item& item)
{
	...
	{
		Item* ptr = new Item[numberItems + 1];
		for (int i = 0; i < numberItems; ++i)
		{
			ptr[i] = items[i];
		}
		ptr[numberItems++] = item;
		delete[] items;
		items = ptr;
	}
}
...
void CourierCompany::printCC() const
{
	...
	cout <<"The number of items is: "<< numberItems << endl;
}